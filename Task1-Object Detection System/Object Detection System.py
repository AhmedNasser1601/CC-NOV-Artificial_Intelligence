...
od.download("https://www.kaggle.com/datasets/aruchomu/data-for-yolo-v3-kernel")
...
imgNames = [
    'Scenarios/scene1.jpg',
    'Scenarios/scene2.jpg',
    'Scenarios/scene3.jpg',
    'Scenarios/scene4.jpg',
    'Scenarios/scene5.jpg'
]
...
def fixed_padding(inputs, kernel_size, data_format):
    ...
    if data_format == 'channels_first':
        padded_inputs = tf.pad(inputs, [[0, 0], [0, 0], [pad_beg,
                               pad_end], [pad_beg, pad_end]])
    else:
...
def darknet53_residual_block(inputs, filters, training, data_format, strides=1):
    ...
    inputs = conv2d_fixed_padding(inputs, filters=filters,
    ...
    inputs = conv2d_fixed_padding(inputs, filters=2 * filters,
...
def yolo_layer(inputs, n_classes, anchors, img_size, data_format):
    ...
    grid_shape = (shape[2:4] if data_format == 'channels_first'
                   else shape[1:3])
    if data_format == 'channels_first':
        inputs = tf.transpose(inputs, [0, 2, 3, 1])
    inputs = tf.reshape(inputs, [-1, n_anchors * grid_shape[0]
                        * grid_shape[1], 5 + n_classes])

    strides = (img_size[0] // grid_shape[0], img_size[1]
               // grid_shape[1])

    (box_centers, box_shapes, confidence, classes) = tf.split(inputs,
            [2, 2, 1, n_classes], axis=-1)
...
def upsample(inputs, out_shape, data_format):
    ...
    inputs = tf.image.resize_nearest_neighbor(inputs, (new_height,
            new_width))
...
def non_max_suppression(inputs, n_classes, max_output_size, iou_threshold, confidence_threshold):
    ...
    boxes_dicts = []
    for boxes in batch:
        boxes = tf.boolean_mask(boxes, boxes[:, 4]
        ...
        for cls in range(n_classes):
            ...
            if mask_shape.ndims != 0:
                ...
                indices = tf.image.non_max_suppression(boxes_coords,
                        boxes_conf_scores, max_output_size,
                        iou_threshold)
...
class Yolo_v3:
    ...
    def __call__(self, inputs, training):
        with tf.variable_scope('yolo_v3_model'):
            ...
            inputs = conv2d_fixed_padding(route, filters=256,
                    kernel_size=1, data_format=self.data_format)
            inputs = batch_norm(inputs, training=training,
                                data_format=self.data_format)
            inputs = tf.nn.leaky_relu(inputs, alpha=_LEAKY_RELU)
...
def draw_boxes(imgNames, boxes_dicts, class_names, model_size):
    ...
    for (num, img_name, boxes_dict) in zip(range(len(imgNames)), imgNames, boxes_dicts):
        ...
        font = ImageFont.truetype(font='data-for-yolo-v3-kernel/futur.ttf', size=(img.size[0] + img.size[1]) // 100)
...
for img in imgNames: display(Image.open(img))


# In[23]:


batch_size = len(imgNames)
batch = load_images(imgNames, model_size=_MODEL_SIZE)
class_names = load_class_names('data-for-yolo-v3-kernel/coco.names')

model = Yolo_v3(n_classes=len(class_names), model_size=_MODEL_SIZE,
                max_output_size=10, iou_threshold=0.5, confidence_threshold=0.5)
